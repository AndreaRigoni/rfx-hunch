...
class GAN0(GAN):
    ...
    def set_optimizers(self, opt=tf.keras.optimizers.Adam, dfactor=1e-2):
        self.optimizers = [
            opt(dfactor),
            opt(dfactor)
        ]
    ...
    def set_model(self, training=True):
        ...
        self.inference_net = tf.keras.Sequential( [
            ...
            tf.keras.layers.Conv2D(filters=16*scale, kernel_size=(2,3), strides=(1, 1), padding='SAME'),
        ...
        self.generative_net = tf.keras.Sequential( [
            ...
            tf.keras.layers.Dense(units=latent_dim, use_bias=False),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.LeakyReLU(),
            tf.keras.layers.Dense(feature_dim * 10 * scale, use_bias=False),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.LeakyReLU(),
            tf.keras.layers.Reshape(target_shape=(2, int(feature_dim/2), int(10*scale) )),
